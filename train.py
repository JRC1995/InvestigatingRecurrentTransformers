...
def run(args, config, time=0):
    ...
    #model = T.compile(model, dynamic=True)
    #model = model.half()
    ...
    if not args.test:
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            while current_iter < train_data_len:
                ...
                if agent.epoch_scheduler is not None and (current_iter == train_data_len):
                    if epoch >= config["schedule_start"]:
                        if "plateau" in config["scheduler"].lower():
                            agent.epoch_scheduler.step(dev_score)
                        else:
