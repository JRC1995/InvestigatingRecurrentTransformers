...
class TLBEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.tlb_block_num = config["tlb_blocks"]
        self.tlb_ratio = config["tlb_ratio"]

        self.SABlocks = nn.ModuleList([self.encoderBlock(config) for _ in range(self.tlb_block_num * self.tlb_ratio)])
        self.CABlocks = nn.ModuleList([self.crossBlock(config) for _ in range(self.tlb_block_num)])
    ...
    def forward(self, sequence, input_mask):
        ...
        memory_pos = self.memory_position_embeddings(memory_positions.long())
        assert memory_pos.size() == (N, self.memory_size, self.hidden_size)
        key_mask = T.ones(N, self.memory_size).float().to(input_mask.device)
        ...
        for sequence, input_mask in zip(chunked_sequence, chunked_input_mask):
            ...
            layer_num = 0
            for l in range(self.tlb_block_num):
                for r in range(self.tlb_ratio):
                    index = l * self.tlb_ratio + r
                    sequence = self.SABlocks[index](sequence=sequence,
                                                    positions=positions,
                ...
                sequence = self.CABlocks[l](sequence=sequence,
                                            kv_sequence=memory,
                                            positions=None,
                                            kv_positions=None,
                                            input_mask=key_mask,
                                            ...
                                            position_encodings=position_encodings,
                                            kv_position_encodings=memory_pos,
                                            layer_num=10000,
                                            xpos=False)["sequence"]
            ...
            for l in range(self.update_layer_num):
                memory = self.update_layers[l](sequence=memory,
                                               ...
                                               kv_positions=None,
                                               input_mask=pseudo_input_mask,
                                               query_input_mask=key_mask,
                                               position_encodings=memory_pos,
