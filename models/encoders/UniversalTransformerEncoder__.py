...
class UniversalTransformerEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.halt_layers = nn.Sequential(nn.Linear(self.hidden_size, self.hidden_size),
                                         nn.GELU(),
    ...
    def forward(self, sequence, input_mask):
        ...
        for l in range(L):
            ...
            accu_sequence = (current_halt_prob.view(N, S, 1) * sequence0) + accu_sequence
            ...
            next_sequence = (1 - total_halt_prob.view(N, S, 1)) * sequence + accu_sequence
            # print("l: {}, halt_prob: {}".format(l, halt_prob))

            update_flag = T.where(total_halt_prob >= 0.999,
                                  T.zeros(N, S).float().to(sequence.device),
                                  T.ones(N, S).float().to(sequence.device))
            update_flag = update_flag * input_mask.view(N, S)
