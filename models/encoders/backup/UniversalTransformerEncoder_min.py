...
class UniversalTransformerEncoder(nn.Module):
    ...
    def forward(self, sequence, input_mask):
        ...
        for l in range(L):
            ...
            cat_sequence = T.cat([sequence0, sequence], dim=-1)
            halt_prob_ = self.halt_layers(cat_sequence).squeeze(-1)
            halt_prob_ = input_mask * halt_prob_ + (1-input_mask) * 99999
            halt_prob = T.min(halt_prob_, dim=1)[0]
