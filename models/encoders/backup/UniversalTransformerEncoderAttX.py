...
class UniversalTransformerEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.cat_compress = nn.Sequential(nn.Linear(2 * self.hidden_size, self.hidden_size),
                                          nn.GELU())
    ...
    def forward(self, sequence, input_mask):
        ...
        for l in range(L):
            ...
            attn = F.softmax(self.attn_scorer(cat_sequence).squeeze(-1) * input_mask + (1 - input_mask) * -99999, dim=1)
            ...
            #print("total_halt_prob: ", total_halt_prob)
