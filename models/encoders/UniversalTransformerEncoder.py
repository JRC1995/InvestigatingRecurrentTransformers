...
class UniversalTransformerEncoder(nn.Module):
    ...
    def forward(self, sequence, input_mask):
        ...
        if not self.config["causal"]:
            ...
            N, S, D = sequence.size()
            input_mask_no_end = T.cat([input_mask[:, 1:].clone(),
                                       T.zeros(N, 1).to(input_mask.dtype).to(input_mask.device)], dim=1)
            END_mask = input_mask - input_mask_no_end
            END_mask = END_mask.unsqueeze(-1)
        ...
        if self.causal:
            denom = T.cumsum(input_mask, dim=1).view(N, S, 1)
        ...
        for l in range(L):
            ...
            #force_halt = T.zeros(N).float().to(sequence.device)
            ...
            if self.attn_based_halt:
                ...
                if self.causal:
                    global_state0 = (T.cumsum(sequence0, dim=1) * input_mask.unsqueeze(-1)) / denom
                    global_state1 = (T.cumsum(sequence, dim=1) * input_mask.unsqueeze(-1)) / denom
                else:
