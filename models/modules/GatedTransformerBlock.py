...
class GatedTransformerBlock(nn.Module):
    def __init__(self, config):
        ...
        nn.init.xavier_uniform_(self.FFN_gate[0].weight)
        nn.init.xavier_uniform_(self.FFN_gate[3].weight)
        nn.init.normal_(self.FFN_gate[0].bias, std=1e-6)
        nn.init.normal_(self.FFN_gate[3].bias, std=1e-6)
    ...
    def forward(self, sequence, kv_sequence, positions, kv_positions,
                ...
                halt_sequence=None, halt_position_encodings=None,
                position_encodings=None, kv_position_encodings=None, xpos=True):
        """
        ...
        sequence = self.FFN(sequence) + res2
