...
class MHA(nn.Module):
    ...
    def forward(self, sequence, positions, input_mask, layer_num,
        ...
        if self.config["causal"]:
            ...
            attention_mask = T.where(attention_mask == 1,
                                     T.zeros(N, S, S).to(attention_mask.dtype).to(input_mask.device),
                                     T.ones(N, S, S).to(attention_mask.dtype).to(input_mask.device) * (-99999))
        else:
            attention_mask = T.where(input_mask.unsqueeze(1) == 1,
                                     T.zeros(N, 1, S).to(input_mask.dtype).to(input_mask.device),
                                     T.ones(N, 1, S).to(input_mask.dtype).to(input_mask.device) * (-99999))
        ...
        A = self.dropout(F.softmax(A + attention_mask, dim=-1))
        assert A.size() == (N, self.heads, S, S2)
